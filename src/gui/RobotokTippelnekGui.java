...
public class RobotokTippelnekGui extends javax.swing.JFrame {
    ...
    private void btnGeneralActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGeneralActionPerformed
        int minVal = (int)numMin.getValue();
        int maxVal = (int)numMax.getValue();
        ...
        if(minVal != maxVal){
            if(minVal>maxVal){
                ...
                minVal = maxVal;
                maxVal = tmp;
                numMin.setValue(minVal);
                numMax.setValue(maxVal);
            }
            
            int i = 0;
            int iMax = !jCheckBox1.isSelected() ? 1 : 2;
            int[] numList = {-1, -2};

            while(i<iMax){
                numList[i] = (int)(Math.random()*(maxVal-minVal+1) + minVal);
                if(numList[0] != numList[1]){
                    i++;
                }
            }
            
            txtVelSzam1.setText(numList[0] + "");
            txtVelSzam2.setText(numList[iMax-1] + "");
        }
        else{
            JOptionPane.showMessageDialog(rootPane, "A minimum és maximum szám nem lehet ugyan az!", "Warning", JOptionPane.WARNING_MESSAGE);
        }
